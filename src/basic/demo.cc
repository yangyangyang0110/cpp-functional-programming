...
bool is_not_space(const char &c) noexcept { return c != ' '; }
...
std::string trim(std::string s) { return trim_right(trim_left(std::move(s))); }

double average_score(const std::vector<float> &scores) noexcept
{
    return std::accumulate(scores.cbegin(), scores.cend(), 0.0f) / double(scores.size());
}
