...
template <typename T> struct Image : public MoveOnly {
    T *data;
...
template <typename T> struct Image<T[]> : public MoveOnly {
    T *data;
    size_t size;
    ...
    {
        os << "[";
        for (size_t i = 0; i < ins.size; ++i) { os << ins.data[i] << " "; }
        os << "], size: " << ins.size;
        return os;
    }
};
...
void testBasic()
{
    ...
    std::vector<Response> responses = {{.is_error = true, .data = "200"},
                                       {.is_error = false, .data = "400"},
                                       {.is_error = true, .data = "222"},
                                       {.is_error = false, .data = "500"}};
    print(responses);

    auto it = std::remove_if(responses.begin(), responses.end(),
