...
template <typename Prediction, typename T = Person> class Filter
{
private:
    std::vector<T> arr_;
...
public:
    ...
    auto &operator++()
    {
        ...
        current_position_ = std::find_if(current_position_ + 1, arr_.end(), prediction_);
        return *this;
    }
};
...
template <typename UnaryOperator, typename T = Person> class Transform
{
private:
    std::vector<T> arr_;
    decltype(arr_.begin()) current_position_;
...
void test_transform()
{
    Transform persons(
        ...
        [](const Person &item) {
            Person person = item;
            std::transform(item.name.cbegin(), item.name.cend(), person.name.begin(),
