...
public:
    ...
    template <typename TimeType = std::chrono::milliseconds> long long int Cost()
    {
        return std::chrono::duration_cast<TimeType>(std::chrono::system_clock::now() -
                                                    now_)
            .count();
    }
};
...
template <typename T = unsigned int> class FibCache
{
private:
    T last_;
    T previous_;
...
// Generalized memory
template <typename Result, typename... Args> auto make_memoized(Result (*f)(Args...))
{
