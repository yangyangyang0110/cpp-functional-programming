...
template <typename Arg> void echo(Arg &&arg)
{
    std::cout << std::forward<Arg>(arg) << std::endl;
}

template <typename F, typename... Args> void invokeResult(F f, Args &&...args)
{
    f(std::forward<Args>(args)...);
}

template <typename F, typename... Args> void invokeFunctor(F f, Args &&...args)
{
    std::invoke(f, std::forward<Args>(args)...);
}

template <typename F, typename... Args> void invokeTupleFunctor(F f, Args &&...args)
{
    auto all_args = std::tuple<std::decay_t<Args>...>(std::forward<Args>(args)...);
...
// 判断是否可以调用
template <typename F, typename... CapturedArgs> class Curried
{
public:
    // using CaptureArgsTuple = std::tuple<std::decay_t<CapturedArgs>...>;

    template <typename... Args> static auto capture_by_copy(Args &&...args)
    {
    ...
    Curried(F f, CapturedArgs... args)
        : function_(f)
        , args_tuple_(capture_by_copy(std::move(args)...))
    {
    }

    Curried(F f, std::tuple<CapturedArgs...> args_tuple)
        : function_(f)
        , args_tuple_(std::move(args_tuple))
    {
    }
...
void test()
{
    ...
    invokeResult([](int x, int y) { std::cout << x << " " << y << std::endl; }, 1, 2);
    std::apply([](int x, int y) { std::cout << x << " " << y << std::endl; },
               std::tuple(1, 2));
...
void test_curried()
{
    ...
    // 对于比较重的资源可以使用值的引用.
    Curried c2([]<typename T>(std::ostream &os, const std::string &t0, T t1, auto t2) {
        os << t0 << " " << t1 << " " << t2;
    });
    // 引用 + 常引用 + 值传递.
    ...
    // TODO: 不能判断何时执行, 这个问题在添加模版参数后比较明显(模版参数实例化后的类型).
    // guess -> T: const std::string &
